...
//! };
//! use sylow::{G1Affine, Fp, Fp2, Fr, Gt, G1Projective, G2Projective, glued_pairing};
...
//!     Fp::from_be_bytes(<&[u8; 32]>::try_from(&input[..32]).unwrap()).into_option().ok_or(
...
//!     let fr = Fp::from(Fr::from_be_bytes(<&[u8; 32]>::try_from(&input[64..96]).unwrap()).unwrap());
//!
...
//!                 Fp::from_be_bytes(<&[u8; 32]>::try_from(slice).unwrap()).into_option().ok_or(
//!                     Error::Bn128FieldPointNotAMember)
//!             };
//!             let ax = read_fp_at(0)?;
//!             let ay = read_fp_at(1)?;
