...
class FieldTestMetaclass(type):
    def __new__(cls, name, bases, attrs, operation=mul, D=2):
        fields = [Fp, Fp2, Fp6]
                    ...
                    for value in test_values:
                        ...
                        for j in [a,b]:
                            if isinstance(j, Iterable):
                                assertEqual(len(j), D, f"Input test {j} is not correct length (D={D})")
                        result = operation(a,b)
                        assertTrue(result in f, f"{fn} multiplication result not in field")
                        assertEqual(result, a * b, f"{fn} multiplication not consistent")
                        ...
                        print(f"Debug {fn}: result = {self.limited_repr(result, self.D)}")
                        print("-" * 50)
                # print(f"{field_name} passed!")
                return test_method

            attrs[f'test_{field_name}_multiplication'] = create_test_method(field, field_name)
...
class FieldMultiplication(metaclass=FieldTestMetaclass, D=1):
    pass

class QuadraticFieldMultiplication(metaclass=FieldTestMetaclass, D=2):
    pass

class SexticFieldMultiplication(metaclass=FieldTestMetaclass, D=3):
    pass
...
for instance in [FieldMultiplicationTest()]:#, QuadraticFieldMultiplicationTest(), SexticFieldMultiplicationTest()]:
