...
impl GroupTrait<2,2, Fp2> for G2Affine {
    ...
    fn sign_message<E: Expander>(exp: &E, msg: &[u8], private_key: Fp2) -> Result<Self, GroupError> {
        unimplemented!()
    }
}
impl GroupTrait<2, 2, Fp2> for G2Projective {
    ...
    fn rand<R: CryptoRngCore>(rng: &mut R) -> Self {
        let rando = <Fp as FieldExtensionTrait<1,1>>::rand(rng).value().to_le_bytes();
...
impl G2Projective {
    pub(crate) fn new(v: [Fp2; 3]) -> Result<Self, GroupError> {
        ...
        match bool::from(is_on_curve) {
            true => {
                match bool::from(is_torsion_free) {
                    true => Ok(Self {
                           x: v[0],
                           y: v[1],
                           z: v[2],
